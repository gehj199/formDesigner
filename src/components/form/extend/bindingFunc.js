...
export default class BindingFunc extends Component{
  ...
  handleClick = (e) => {
    if((this.refs.saveType.value).toLowerCase()=="modeltablename"){
      ...
      var sysTable=/^sys\_/gi;
      if(!reg.test(checkValues))
          $.messager.alert(message.checkResult,message.falseVariable,'error');
      else if(sysTable.test(checkValues))
          $.messager.alert(message.checkResult,message.sysFormat,'error');
      else{
        var href = (window.location.href).match(/(\S*)(?=formDesigner)/);
      ...
      $.ajax({
        ...
        success: function (data) {
          if(data){
            ...
            $.messager.alert(message.checkResult,message.notFound,'error');
       },
       error:function(XMLHttpRequest, textStatus, errorThrown){
            if(XMLHttpRequest.status!=200)
              $.messager.alert(message.checkResult,"URL Error",'error');
            else if(XMLHttpRequest.readyState!=4)
              $.messager.alert(message.checkResult,"NO response",'error');
　　        else if(textStatus=='parsererror')
              $.messager.alert(message.checkResult,errorThrown.message,'error');
       }
     });
    }
    else{
